...
class RegisterSimulation extends Simulation {
  ...
  private val scn = scenario("RegisterSimulation")
    .exec(
      http("request_0")
        .get("/")
        .headers(headers_0)
        .resources(
          http("request_1")
            .get("/assets/index-QligUd_N.js")
            .headers(headers_1),
          http("request_2")
            .get("/assets/index-CqzyyKSk.css")
            .headers(headers_2),
          http("request_3")
            .get("/tri-billiard.svg")
            .headers(headers_3)
        ),
      pause(10),
      http("request_4")
        ...
        .resources(
          http("request_5")
            ...
            .headers(headers_5)
            .body(RawFileBody("io/gatling/demo/registersimulation/0005_request.json"))
        )
    )
