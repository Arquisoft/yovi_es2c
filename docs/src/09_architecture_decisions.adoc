...
=== ADR-07: Uso de AWS como infraestructura de despliegue
...
==== Decisión
Utilizar Amazon Web Services (AWS) como proveedor de infraestructura
para desplegar los componentes del sistema.

La arquitectura se desplegará sobre servicios cloud gestionados,
permitiendo:
...
==== Alternativas consideradas
1. Otros proveedores cloud (Azure).
2. Otros proveedores cloud (Google Cloud).
...
- El equipo ya ha trabajado previamente con Azure en otros proyectos,
  por lo que adoptar AWS amplía la experiencia en entornos multi-cloud.
...
- Soporte para automatización de despliegues.

Negativas:
- Dependencia de proveedor externo.
- Costes variables según uso.
- Necesidad de configurar correctamente seguridad (IAM, redes, firewall).
